...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> countryArrayList = new HashMap<String, Integer>();
		int totalEarthquakes = 0;
		int landEarthquakes = 0;
		int oceanEarthquakes = 0;
		for (Marker cMarker : countryMarkers) {
			String nameString = (String)cMarker.getProperty("name");
			for (Marker eMarker : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker)eMarker;
				String countryString = (String) eMarker.getProperty("country");

				if (em.isOnLand && countryString == nameString) {
					if (countryArrayList.containsKey(countryString)) {
						int quakeCounter = (int) countryArrayList.get(countryString) + 1;
						countryArrayList.put(countryString, quakeCounter);
					} else {
						countryArrayList.put(countryString, 1);
					}
					landEarthquakes ++;
				}
			}
		...
		System.out.println(landEarthquakes);
		for (Map.Entry<String,Integer> entry : countryArrayList.entrySet()) {
